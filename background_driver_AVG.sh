...
do
	...
	(cd $hydrofolder
	./VISHNew.e IEOS=7 Edec=0.18 T0=0.6 vis=`echo $etaBYsval` IINIT=2 iEin=1 iLS=130 factor=1.0 >> $hydrofolder/results/RunRecord_results-avg_etaBYs_`echo $etaBYsval`.txt
	cd $basedirectory)
	echo 'Finished hydro for eta/s =' $etaBYsval 'and results-avg-1.' >> $outfile
	cp $ICsfolder`echo avg`/sd*_block.dat $hydrofolder/results/
	mv $hydrofolder/results $basedirectory/RESULTS/RESULTS_etaBYs_`echo $etaBYsval`/NEW_results-avg-1
	echo 'Moved hydro results to' $basedirectory/RESULTS/RESULTS_etaBYs_`echo $etaBYsval`/NEW_results-avg-1 >> $outfile
	echo '' >> $outfile
done
