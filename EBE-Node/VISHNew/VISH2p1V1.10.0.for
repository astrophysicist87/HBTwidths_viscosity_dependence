...
CSHEN===EOS from tables end====================================================
       ...
       Common /IVisflagINPUT/ IVisflagINPUT
...
C------ Chris Plumberg added a flag for eta/s(T) -----------------------
      Read(1,*) IVisflagINPUT
...
C------ Chris Plumberg added this line -----------------------
      IVisflag = IVisflagINPUT

            Write (*,*) "Have:", "IEOS=", IEOS, "A=", A, ! write out parameter for a check
     &    "IInit=", IInit, "IVisflag=", IVisflag, "dT=", dT_1,
...
C      ViscousCTemp = ViscousC
      ...
      if(IVisflag.eq.1) then
        result = 0.08d0
      else if(IVisflag.eq.2) then
        result = 0.08d0
        if(TT.gt.TTR) then
          result = a2QGP*ratTT2 + a1QGP*ratTT + a0QGP
        end if
      else if(IVisflag.eq.3) then
        result = 0.08d0
        if(TT.lt.TTR) then
          result = a2HRG*ratTT2 + a1HRG*ratTT + a0HRG
        end if
      else if(IVisflag.eq.4) then
        result = 0.08d0
        if(TT.lt.TTR) then
          result = a2HRG*ratTT2 + a1HRG*ratTT + a0HRG
        else if(TT.gt.TTR) then
          result = a2QGP*ratTT2 + a1QGP*ratTT + a0QGP
        end if
      end if
