...
def generate_all_self_plots():
	...
	ax1.axis([xlower, xupper, ylower, yupper])
	ax1.set_xticklabels([])
	ax1.set_ylabel(r'$\left( R^2_{s} \!\right) _{SSH}/\left( R^2_{s} \!\right) _{DEA} - 1$ (%)', {'fontsize': plotfontsize})
	ax1.text(0.05, 0.85,'(a)', transform=ax1.transAxes, fontsize=plotfontsize + 5)
	...
	ax2.plot(LHLQdata[:,0], relf(LHLQdata[:,2],LHLQdataDEA[:,2]), linestyle='-', color='red', linewidth=3, label='LH-HQ')
	ax2.plot(LHHQdata[:,0], relf(LHHQdata[:,2],LHHQdataDEA[:,2]), linestyle=':', color='blue', linewidth=3, label='LH-HQ')
	ax2.plot(HHLQdata[:,0], relf(HHLQdata[:,2],HHLQdataDEA[:,2]), linestyle='-.', color='black', linewidth=3, label='HH-LQ')
	ax2.plot(HHHQdata[:,0], relf(HHHQdata[:,2],HHHQdataDEA[:,2]), linestyle='--', color='green', linewidth=3, label='HH-HQ')
	
	# update axes limits and axes labels
	ax2.axis([xlower, xupper, ylower, yupper])
	ax2.set_xticklabels([])
	ax2.set_ylabel(r'$\left( R^2_{o} \!\right) _{SSH}/\left( R^2_{o} \!\right) _{DEA} - 1$ (%)', {'fontsize': plotfontsize})
	ax2.yaxis.set_label_position('right')
	ax2.yaxis.tick_right()
	ax2.yaxis.set_ticks_position('both')
	ax2.text(0.05, 0.85,'(b)', transform=ax2.transAxes, fontsize=plotfontsize + 5)
	...
	ax3.plot(HHLQdata[:,0], relf(HHLQdata[:,3],HHLQdataDEA[:,3]), linestyle='-.', color='black', linewidth=3, label='HH-LQ')
	ax3.plot(HHHQdata[:,0], relf(HHHQdata[:,3],HHHQdataDEA[:,3]), linestyle='--', color='green', linewidth=3, label='HH-HQ')
	
	# update axes limits and axes labels
	ax3.axis([xlower, xupper, ylower, yupper])
	ax3.set_xticklabels(['0.0', '0.5', '1.0', '1.5'])
	ax3.set_xlabel(r'$K_T$ (GeV)', {'fontsize': plotfontsize + 5})
	ax3.set_ylabel(r'$\left( R^2_{l} \!\right) _{SSH}/\left( R^2_{l} \!\right) _{DEA} - 1$ (%)', {'fontsize': plotfontsize})
	#ax3.text(0.85, 0.1,'(c)', transform=ax3.transAxes, fontsize=plotfontsize + 5)
	...
	ax4.plot(LHLQdata[:,0], onezero*relf(LHLQdata[:,4],LHLQdataDEA[:,4]), linestyle='-', color='red', linewidth=3, label='LH-HQ')
	ax4.plot(LHHQdata[:,0], onezero*relf(LHHQdata[:,4],LHHQdataDEA[:,4]), linestyle=':', color='blue', linewidth=3, label='LH-HQ')
	...
	ax4.axis([xlower, xupper, ylower, yupper])
	ax4.set_xlabel(r'$K_T$ (GeV)', {'fontsize': plotfontsize + 5})
	ax4.set_ylabel(r'$\left( R^2_{ol} \!\right) _{SSH}/\left( R^2_{ol} \!\right) _{DEA} - 1$ (%)', {'fontsize': plotfontsize})
	ax4.yaxis.set_label_position('right')
	ax4.yaxis.tick_right()
	ax4.yaxis.set_ticks_position('both')
	#ax4.text(0.85, 0.1,'(d)', transform=ax4.transAxes, fontsize=plotfontsize + 5)
