	...
	if abs(ebs-0.08) < eps:
		for TVidx in xrange(1,5):
			...
			for order in xrange(maxorder+1):
				iKT = 0
				for KT in linspace(0,2,nKT):
					outfileCOS = direc + '/NEW_TDEP_V%(TV)d_EbE_HBT_and_SV_KT_%(KT)0.1f_GeV_%(nevs)devs_COSneq%(ord)d' % {"TV": TVidx, "KT": KT, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
					outfileSIN = direc + '/NEW_TDEP_V%(TV)d_EbE_HBT_and_SV_KT_%(KT)0.1f_GeV_%(nevs)devs_SINneq%(ord)d' % {"TV": TVidx, "KT": KT, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
	...
	if abs(ebs-0.08) < eps:
		for TVidx in xrange(1,5):
			...
			for order in xrange(maxorder+1):
				iKT = 0
				for KT in linspace(0,2,nKT):
					inCOSfile = direc + '/NEW_TDEP_V%(TV)d_EbE_HBT_and_SV_KT_%(KT)0.1f_GeV_%(nevs)devs_COSneq%(ord)d' % {"TV": TVidx, "KT": KT, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
					inSINfile = direc + '/NEW_TDEP_V%(TV)d_EbE_HBT_and_SV_KT_%(KT)0.1f_GeV_%(nevs)devs_SINneq%(ord)d' % {"TV": TVidx, "KT": KT, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
					...
					iKT += 1
				outCOSfile = direc + '/NEW_TDEP_V%(TV)d_complete_FOsurface_properties_etaBYs_%(ebs)0.2f_%(nevs)devs_COSneq%(ord)d' % {"TV": TVidx, "ebs": ebs, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
				outSINfile = direc + '/NEW_TDEP_V%(TV)d_complete_FOsurface_properties_etaBYs_%(ebs)0.2f_%(nevs)devs_SINneq%(ord)d' % {"TV": TVidx, "ebs": ebs, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
		...
		for order in xrange(maxorder+1):
			...
			for KT in linspace(0,2,nKT):
				...
				allSINresults[iKT, :] = hstack((KT, mean(SINdata,0), var(SINdata,0)))
				iKT += 1
			outCOSfile = direc + '/complete_FOsurface_properties_etaBYs_%(ebs)0.2f_%(nevs)devs_COSneq%(ord)d' % {"ebs": ebs, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
			outSINfile = direc + '/complete_FOsurface_properties_etaBYs_%(ebs)0.2f_%(nevs)devs_SINneq%(ord)d' % {"ebs": ebs, "nevs": nevs, "ord": order} + df_stem + abs_stem + '.dat'
			print 'Saving to', outCOSfile
			savetxt(outCOSfile, allCOSresults, fmt='%f')
			print 'Saving to', outSINfile
			savetxt(outSINfile, allSINresults, fmt='%f')
...
def cleanup(df_stem):
	if abs(ebs-0.08) < eps:
		for TVidx in xrange(1,5):
			...
			direc='RESULTS_etaBYs_%(ebs)0.2f/NEW_TDEP_V%(TV)d' % {"ebs": ebs, "TV": TVidx}
			filePattern = direc + '/NEW_TDEP_V%(TV)d_EbE_HBT_and_SV_KT_*_GeV_1000evs_*neq[0-9]' % {"TV": TVidx} + df_stem + abs_stem + '.dat'
